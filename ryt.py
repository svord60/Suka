...
# ========== ПРОВЕРКА CRYPTOBOT ОПЛАТЫ (РЕАЛЬНАЯ) ==========
...
async def check_crypto_payment(callback: types.CallbackQuery):
    ...
    if result["success"]:
        if result["status"] == "paid":
            ...
            await callback.answer(
                "✅ Оплата подтверждена! Товар будет доставлен в течение 15 минут - 3 часа.",
                show_alert=True
            )
...
# ========== КНОПКИ УПРАВЛЕНИЯ ДЛЯ /check_номер ==========
...
# ========== ОСТАЛЬНЫЕ РАЗДЕЛЫ АДМИН ПАНЕЛИ ==========
...
# ========== КОМАНДЫ АДМИНА (старые, для совместимости) ==========
